...
def test_spec_resolution():
    ...
    hspec = ps.PlottingSpectra(68, base+halo, None, None, savefile=base+halo+"/snapdir_068/spectra1024.hdf5")
    hspec.plot_vel_width("Si",2, color="red")
    hspec = ps.PlottingSpectra(68, base+halo, None, None, savefile=base+halo+"/snapdir_068/spectra2048.hdf5")
    ...
    hspec2.plot_vel_width("Si", 2, color="black")
    del hspec2
...
for i in (0,1,2):
    plot_vel_width_sim(0,zz[i],ff=True, HI_cut = 10**20.3)
    vel_data.plot_prochaska_2008_data()
    save_figure(path.join(outdir,"cosmo512_vel_widthz"+str(zz[i])))
    plt.clf()

for ii in (0,3):
    plot_vel_width_sim(ii,60,HI_cut = 10**20.3)
    vel_data.plot_prochaska_2008_data()
