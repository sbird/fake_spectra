...
def compute_absorption(xbins, rho, vel, temp, line):
    ...
    dzgrid   = box100 / nbins  # bin size (kpc)
    ...
    tau_prefac = rscale*line.sigma_X*C*dzgrid/np.sqrt(math.pi)
...
def voigt(a, x):
    ...
    if x2 < 1.0e-10:
        ...
        profile = T1 - a/np.sqrt(math.pi)/x2*(T1*T1*(4.0*x2**2 + 7.0*x2 + 4.0 + T2) - T2 -1.0)
    return profile
...
class MetalLines:
    ...
    def __init__(self,los_table, snapshot, cloudy_dir):
        ...
        (self.rho_out, self.temp_out, self.vel_out, self.rho_metal, self.temp_metal, self.vel_metal) = SPH_Interpolate(f["PartType0"], los_table, 1024, f["Header"].attrs["BoxSize"])
        #Generate cloudy tables
        self.cloudy = convert_cloudy.CloudyTable(cloudy_dir)
        self.redshift = f["Header"].attrs["Redshift"]

    def get_lines(self, species, ion):
        """Get the optical depth for a particular species number for all spectra:
           (C, N, O, Ne, Mg, Si, Fe)
           and ion number
        """
        metal_density = self.rho_metal[:, species]
        ...
        ions = self.cloudy.interpolate(self.redshift, tot_met, metal_density, species)
        ion_density = ions[:,:,ion]* metal_density
        tau_metal = compute_absorption(ion_density, self.temp_metal[:, species], self.vel_metal[:,species],line_data.lines[species+2,ion])
        return tau_metal
